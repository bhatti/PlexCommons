...
public class ClassParser {
	...
	private static boolean verbose = "true".equals(System
			.getProperty("verbose"));

	public ClassParser() {
	}

	public ClassFile[] process(String filename) throws java.io.IOException,
			ClassFormatError {
		return process(getStream(filename));
	}

	public ClassFile[] process(DataInputStream in) throws java.io.IOException,
			ClassFormatError {
		List<ClassFile> vecClassFiles = new ArrayList<ClassFile>();
		ClassFile cf = new ClassFile(in);
		vecClassFiles.add(cf);
		...
		for (int i = 0; i < innerClasses.length; i++) {
			...
			for (int j = 0; j < arrcf.length; j++) {
				if (vecClassFiles.indexOf(arrcf[j]) == -1)
					vecClassFiles.add(arrcf[j]);
			}
		}
		return vecClassFiles.toArray(new ClassFile[vecClassFiles.size()]);
	}

	public class ClassFile {
		public ClassFile(String filename) throws java.io.IOException,
				ClassFormatError {
			this(getStream(filename));
		}

		public ClassFile(DataInputStream in) throws java.io.IOException,
				ClassFormatError {
			// magic = in.readInt() & 0xFFFFFFFF;
			magic = in.readInt();
			if (magic != MAGIC) {
				throw new ClassFormatError("Illegal magic code "
						+ Integer.toHexString((int) magic));
			}
		...
		public String[] getInnerClasses(String type) {
			if (verbose)
				System.err.println("@@@@@@@@@@@@@@ Finding inner class for "
						+ type);
			String[] types = getTypes();
			List<String> vecInner = new ArrayList<String>();
			for (int i = 0; i < types.length; i++) {
				if (types[i].startsWith(type + '$')
					...
					if (vecInner.indexOf(types[i]) == -1) {
						if (verbose)
							System.err
									.println("@@@@@@@@@@@@@@ Found Innerclass "
											+ types[i]);
						vecInner.add(types[i]);
					}
				}
			}
			return vecInner.toArray(new String[vecInner.size()]);
		}

		public String[] getConstants() {
			List<String> vecConst = new ArrayList<String>();
			getConstants(vecConst);
			return vecConst.toArray(new String[vecConst.size()]);
		}

		public void getConstants(List<String> vecConst) {
			for (int i = 1; i < constant_pool_count; i++) {
		...
		public String[] getTypes() {
			...
			getTypes(vecTypes);
			return vecTypes.toArray(new String[vecTypes.size()]);
		}

		public void getTypes(List<String> vecTypes) {
			if (verbose) {
				for (int i = 1; i < constant_pool_count; i++) {
					if (constant_pool[i] != null) {
			...
			/*
			 * for (int i=1; i<constant_pool_count; i++) { if
			 * (constant_pool[i].tag == cp_info.CONSTANT_Utf8) {
			 * System.err.println("utf at constant_pool[" + i + "]=" +
			 * getUtf8(constant_pool, i)); } } for (int i=0; i<attributes_count;
			 * i++) { System.err.println("attributes[" + i + "]=" +
			 * attributes[i]); } for (int i=1; i<constant_pool_count; i++) {
			 * System.err.println("*constant_pool[" + i + "]=" +
			 * constant_pool[i]); }
			 */
			addClass(constant_pool, this_class, vecTypes);
			if (super_class > 0)
				addClass(constant_pool, super_class, vecTypes);
			for (int i = 0; i < interfaces_count; i++) {
				addClass(constant_pool, interfaces[i], vecTypes);
			}
			for (int i = 0; i < fields_count; i++) {
				// addNondup(getUtf8(constant_pool, fields[i].name_index),
				// vecTypes);
				addNondup(getUtf8(constant_pool, fields[i].descriptor_index),
						vecTypes);
				for (int j = 0; j < fields[i].attributes_count; j++) {
					add(constant_pool, fields[i].attributes_count,
							fields[i].attributes, vecTypes);
				}
			}
			for (int i = 0; i < methods_count; i++) {
				// addNondup(getUtf8(constant_pool, methods[i].name_index),
				// vecTypes);
				addNondup(getUtf8(constant_pool, methods[i].descriptor_index),
						vecTypes);
				for (int j = 0; j < methods[i].attributes_count; j++) {
					add(constant_pool, methods[i].attributes_count,
							methods[i].attributes, vecTypes);
				}
			}
			...
			for (int i = 1; i < constant_pool_count; i++) {
				...
				case cp_info.CONSTANT_NameAndType:
					add(constant_pool,
							constant_pool[i].asCONSTANT_NameAndType_info(),
							vecTypes);
					break;
				case cp_info.CONSTANT_Fieldref:
					add(constant_pool,
							constant_pool[i].asCONSTANT_Fieldref_info(),
							vecTypes);
					break;
				case cp_info.CONSTANT_Methodref:
					add(constant_pool,
							constant_pool[i].asCONSTANT_Methodref_info(),
							vecTypes);
					break;
				case cp_info.CONSTANT_InterfaceMethodref:
					add(constant_pool,
							constant_pool[i]
									.asCONSTANT_InterfaceMethodref_info(),
							vecTypes);
					break;
				/*
				 * case cp_info.CONSTANT_Utf8: add(constant_pool,
				 * constant_pool[i].asCONSTANT_Utf8_info(),vecTypes); break;
				 */
				}
			}
		}
	...
	public class cp_info {
		...
		public cp_info(DataInputStream in) throws java.io.IOException,
				ClassFormatError {
	...
	public class field_info {
		...
		public field_info(DataInputStream in) throws java.io.IOException,
				ClassFormatError {
			access_flags = in.readUnsignedShort();
	...
	public class method_info {
		...
		public method_info(DataInputStream in) throws java.io.IOException,
				ClassFormatError {
			access_flags = in.readUnsignedShort();
			name_index = in.readUnsignedShort();
			descriptor_index = in.readUnsignedShort();
	...
	public class attribute_info {
		public attribute_info(DataInputStream in) throws java.io.IOException,
				ClassFormatError {
		...
		private synchronized void initValue(cp_info[] constant_pool) {
			if (value != null)
				return;
			tagname = constant_pool[attribute_name_index]
					.asCONSTANT_Utf8_info().value;
			try {
				DataInputStream in = new DataInputStream(
						new ByteArrayInputStream(info));
				if (tagname.equals("Code")) {
					value = new Code_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("Exceptions")) {
					value = new Exceptions_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("InnerClasses")) {
					value = new InnerClasses_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("Synthetic")) {
					value = new Synthetic_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("SourceFile")) {
					value = new SourceFile_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("LocalVariableTable")) {
					value = new LocalVariableTable_attribute(
							attribute_name_index, attribute_length, in);
				} else if (tagname.equals("LineNumberTable")) {
					value = new LineNumberTable_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("Deprecated")) {
					value = new Deprecated_attribute(attribute_name_index,
							attribute_length, in);
				} else if (tagname.equals("ConstantValue")) {
					value = new ConstantValue_attribute(attribute_name_index,
							attribute_length, in);
				}
				in.close();
			} catch (Exception e) {
		...
		public final int attribute_name_index; // valid unsigned 16bit index
												// into
		// constant pool of type CONSTANT_Utf8_info
		// that stores name of attribute
		public final long attribute_length; // length does not include initial
											// six
	...
	public class Code_attribute {
		public class exception_info {
			...
			// ranges
			// in the code array at which the exception handler is active. The
			// value
			// of start_pc must be a valid index into the code array of the
			// opcode
			// of an instruction. The value of end_pc either must be a valid
			// index
			// into the code array of the opcode of an instruction or must be
			// equal
		...
		public Code_attribute(int attribute_name_index, long attribute_length,
				DataInputStream in) throws java.io.IOException,
				ClassFormatError {
		...
		public String toString() {
			...
			buffer.append("exception_table_length=" + exception_table_length
					+ ',');
			for (int i = 0; i < exception_table_length; i++) {
				buffer.append("exception_table[" + i + "]="
						+ exception_table[i] + ',');
			}
			buffer.append('\n');
			buffer.append("attributes_count=" + attributes_count + ',');
	...
	public class Exceptions_attribute {
		...
		public String toString() {
			...
			for (int i = 0; i < number_of_exceptions; i++) {
				buffer.append("exception_index_table[" + i + "]="
		...
		public final int attribute_name_index; // u2 index into constant pool
		// CONSTANT_Utf8_info
		public final long attribute_length; // attribute length excluding
											// initial
		// six bytes
	...
	public class InnerClasses_attribute {
		...
		public class innerclass_info {
			...
			public String toString() {
				StringBuffer buffer = new StringBuffer(1024);
				buffer.append(getClass().getName() + ".toString(");
				buffer.append("inner_class_info_index="
						+ inner_class_info_index + ',');
				buffer.append("outer_class_info_index="
						+ outer_class_info_index + ',');
				buffer.append("inner_name_index=" + inner_name_index + ',');
				buffer.append("inner_class_access_flags="
		...
		// initial six bytes
		public final int number_of_classes; // u2 number of entires in class
											// arr.
	...
	public class Synthetic_attribute {
		public Synthetic_attribute(int attribute_name_index,
				long attribute_length, DataInputStream in)
				throws java.io.IOException, ClassFormatError {
			this.attribute_name_index = attribute_name_index;// in.readUnsignedShort();
			this.attribute_length = attribute_length; // in.readInt();
	...
	public class SourceFile_attribute {
		public SourceFile_attribute(int attribute_name_index,
				long attribute_length, DataInputStream in)
				throws java.io.IOException, ClassFormatError {
			this.attribute_name_index = attribute_name_index;// in.readUnsignedShort();
			this.attribute_length = attribute_length; // in.readInt();
	...
	public class LineNumberTable_attribute {
		...
		// number
		// in the original source file changes at a given point in the code
		// array.
		public class line_number_info {
			...
			// code array at which the code for a new line in the original
			// source
			...
			public final int start_pc; // index into code array at which code
										// for
		...
		public String toString() {
			...
			buffer.append("attribute_name_index=" + attribute_name_index + ',');
			buffer.append("attribute_length=" + attribute_length + ',');
			buffer.append("line_number_table_length="
	...
	public class LocalVariableTable_attribute {
		...
		public LocalVariableTable_attribute(int attribute_name_index,
				long attribute_length, DataInputStream in)
				throws java.io.IOException, ClassFormatError {
			this.attribute_name_index = attribute_name_index;// in.readUnsignedShort();
			this.attribute_length = attribute_length; // in.readInt();
			...
			local_variable_table = new local_variable_table_info[local_variable_table_length];
		...
		public String toString() {
			...
			buffer.append("attribute_name_index=" + attribute_name_index + ',');
			buffer.append("attribute_length=" + attribute_length + ',');
			buffer.append("local_variable_table_length="
	...
	public class Deprecated_attribute {
		public Deprecated_attribute(int attribute_name_index,
				long attribute_length, DataInputStream in)
				throws java.io.IOException, ClassFormatError {
			this.attribute_name_index = attribute_name_index;// in.readUnsignedShort();
			this.attribute_length = attribute_length; // in.readInt();
			...
			if (attribute_length != 0)
				throw new ClassFormatError("Illegal length");
		}
	...
	public class CONSTANT_Float_info {
		...
		/*
		 * public static boolean isNan(int bits) { int s = ((bits >> 31) == 0) ?
		 * 1 : -1; int e = ((bits >> 23) & 0xff); int m = (e == 0) ? (bits &
		 * 0x7fffff) << 1 : (bits & 0x7fffff) | 0x800000; Then the float value
		 * equals the result of the mathematical expression s·m·2e-150. return
		 * false; }
		 */
	...
	public class CONSTANT_Double_info {
		 ...
		 * computed from bits: int s = ((bits >> 63) == 0) ? 1 : -1; int e =
		 * (int)((bits >> 52) & 0x7ffL); long m = (e == 0) ? (bits &
		 * 0xfffffffffffffL) << 1 : (bits & 0xfffffffffffffL) |
		 * 0x10000000000000L;
		 * 
		 * 
		 * Then the floating-point value equals the double value of the
		 * mathematical expression s·m·2e-1075.
		 */
		public String toString() {
			StringBuffer buffer = new StringBuffer(1024);
			buffer.append(getClass().getName() + ".toString(");
	...
	public class CONSTANT_NameAndType_info {
		...
		public final int descriptor_index; // u2 index into constant pool of
											// type
	 ...
	 * values. UTF-8 strings are encoded so that character sequences that
	 * contain only non-null ASCII characters can be represented using only 1
	 * byte per character, but characters of up to 16 bits can be represented.
	 * All characters in the range '\u0001' to '\u007F' are represented by a
	 * single byte: 0 bits 6-0 The 7 bits of data in the byte give the value of
	 * the character represented. The null character ('\u0000') and characters
	 * in the range '\u0080' to '\u07FF' are represented by a pair of bytes x
	 * and y: x: 1 1 0 bits 10-6 y: 1 0 bits 5-0 The bytes represent the
	 * character with the value ((x & 0x1f) << 6) + (y & 0x3f). Characters in
	 * the range '\u0800' to '\uFFFF' are represented by 3 bytes x, y, and z: x:
	 * 1 1 1 0 bits 15-12 y: 1 0 bits 11-6 z: 1 0 bits 5-0 The character with
	 * the value ((x & 0xf) << 12) + ((y & 0x3f) << 6) + (z & 0x3f) is
	 * represented by the bytes. The bytes of multibyte characters are stored in
	 * the class file in big-endian (high byte first) order. There are two
	 * differences between this format and the "standard" UTF-8 format. First,
	 * the null byte (byte) is encoded using the 2-byte format rather than the
	 * 1-byte format, so that Java virtual machine UTF-8 strings never have
	 * embedded nulls. Second, only the 1-byte, 2-byte, and 3-byte formats are
	 * used. The Java virtual machine does not recognize the longer UTF-8
	 * formats.
	 */
	...
	public static String[] getTypes(ClassParser.ClassFile[] classInfo) {
		...
		for (int i = 0; i < classInfo.length; i++) {
			...
			for (int j = 0; j < types.length; j++) {
				if (vecTypes.indexOf(types[j]) == -1)
					vecTypes.add(types[j]);
			}
		}
		return vecTypes.toArray(new String[vecTypes.size()]);
	}
	...
	public static String[] getConstants(ClassParser.ClassFile[] classInfo) {
		...
		for (int i = 0; i < classInfo.length; i++) {
			...
			for (int j = 0; j < consts.length; j++) {
				if (vecConst.indexOf(consts[j]) == -1)
					vecConst.add(consts[j]);
			}
		}
		return vecConst.toArray(new String[vecConst.size()]);
	}
	...
	private static byte[] readBytes(int length, DataInputStream in)
		...
		while (len > 0) {
			...
			if (n == -1)
				throw new EOFException("EOF while reading " + len + "/"
						+ length + " bytes");
			len -= n;
			offset += n;
		}
	...
	private static void add(cp_info[] constant_pool, int attributes_count,
			attribute_info[] attributes, List<String> vec) {
		for (int i = 0; i < attributes_count; i++) {
			if (attributes[i].getTagName(constant_pool).equals("Code")) {
			...
			} else if (attributes[i].getTagName(constant_pool).equals(
					"Exceptions")) {
				Exceptions_attribute attr = attributes[i]
						.asExceptions_attribute();
				for (int j = 0; j < attr.number_of_exceptions; j++) {
					addClass(constant_pool, attr.exception_index_table[j], vec);
				}
			} else if (attributes[i].getTagName(constant_pool).equals(
					"InnerClasses")) {
				InnerClasses_attribute attr = attributes[i]
						.asInnerClasses_attribute();
				// addNondup(getUtf8(constant_pool, attr.attribute_name_index),
				// vec);
				for (int j = 0; j < attr.number_of_classes; j++) {
					if (attr.classes[j].inner_class_info_index > 0)
						addClass(constant_pool,
								attr.classes[j].inner_class_info_index, vec);
					if (attr.classes[j].outer_class_info_index > 0)
						addClass(constant_pool,
								attr.classes[j].outer_class_info_index, vec);
					if (attr.classes[j].inner_name_index > 0) {
						if (attr.classes[j].outer_class_info_index > 0) {
							String outer = getClassName(constant_pool,
									attr.classes[j].outer_class_info_index);
							addNondup(
									'L'
											+ outer
											+ '$'
											+ getUtf8(
													constant_pool,
													attr.classes[j].inner_name_index)
											+ ';', vec);
						}
					}
				}
					...
					"LocalVariableTable")) {
				LocalVariableTable_attribute attr = attributes[i]
						.asLocalVariableTable_attribute();
				for (int j = 0; j < attr.local_variable_table_length; j++) {
					addNondup(
							getUtf8(constant_pool,
									attr.local_variable_table[j].descriptor_index),
							vec);
				}
			} else if (attributes[i].getTagName(constant_pool).equals(
					"Synthetic")) {
				Synthetic_attribute attr = attributes[i]
						.asSynthetic_attribute();
				addNondup(getUtf8(constant_pool, attr.attribute_name_index),
						vec);
			} else if (attributes[i].getTagName(constant_pool).equals(
					"LineNumberTable")) {
				...
				/*
				 * LineNumberTable_attribute attr = attributes[i]
				 * .asLineNumberTable_attribute(); for (int j=0;
				 * j<attr.line_number_table_length; j++) {
				 * System.err.println(line_number_table[j]); }
				 */
			} else if (attributes[i].getTagName(constant_pool).equals(
					"SourceFile")) {
				// SourceFile_attribute attr = attributes[i]
				// .asSourceFile_attribute();
				// System.err.println("Source: " + getUtf8(constant_pool,
				...
				// attributes[i].getTagName(constant_pool) + "]");
			}
		}
	}

	private static void addClass(cp_info[] constant_pool, int n,
			List<String> list) {
	...
	private static void add(cp_info[] constant_pool,
			CONSTANT_Fieldref_info info, List<String> list) {
		addClass(constant_pool, info.class_index, list);
		add(constant_pool,
				constant_pool[info.name_and_type_index]
						.asCONSTANT_NameAndType_info(), list);
	}

	private static void add(cp_info[] constant_pool,
			CONSTANT_Methodref_info info, List<String> list) {
		addClass(constant_pool, info.class_index, list);
		add(constant_pool,
				constant_pool[info.name_and_type_index]
						.asCONSTANT_NameAndType_info(), list);
	...
	private static void add(cp_info[] constant_pool,
			CONSTANT_InterfaceMethodref_info info, List<String> list) {
		addClass(constant_pool, info.class_index, list);
		add(constant_pool,
				constant_pool[info.name_and_type_index]
						.asCONSTANT_NameAndType_info(), list);
	...
	private static void add(cp_info[] constant_pool,
			CONSTANT_NameAndType_info info, List<String> list) {
		addNondup(getUtf8(constant_pool, info.descriptor_index), list);
	}

	@SuppressWarnings("unused")
	private static void add(CONSTANT_Utf8_info info, List<String> list) {
		addNondup(info.value, list);
	}
	...
	private static void addNondup(String value, List<String> list) {
		...
		if (value.charAt(0) == '(') {
			...
			String[] rtypes = rtype == null ? new String[0] : TypesExtractor
					.getComponentType(rtype);
			if (rtypes == null)
				rtypes = new String[0];
		...
		if (debug) {
			if (types == null || types.length == 0) {
				...
				/*
				 * try { throw new Exception("value [" + value + "] failed"); }
	...
	public static void main(String[] args) {
		try {
			...
			if (args[0].endsWith(".class"))
				...
				filepath = TypesExtractor.getPath(args[0]);
			if (filepath == null)
				return;
